...
ctx.beginPath();
ctx.arc(1620, 207.5, 125, 0, 2 * Math.PI);
ctx.lineWidth = 10;
...
ctx.beginPath();
ctx.moveTo(1495, 997.5);
ctx.lineTo(1745, 997.5);
ctx.lineTo(1620, 997.5 - height2);
ctx.closePath();
ctx.lineWidth = 10;
ctx.globalAlpha = 1;
...
var countdown = setInterval(function() {
  ...
  if (seconds >= 600) {
	  ...
	  localStorage.setItem("shapesRightScore", seconds);
	    exerStarted = 0;
...
drawTria(xTria, yTria);
		...
		if (xRect > 1480 && xRect < 1560 && yRect > 400 & yRect < 480) {
			rectGoalColor = "#0f0";
			if (hasLetGo == true) {
		   rectGoalAch = 1;
			}
		   ...
		   rectGoalAch = 0;
			...
			if (xCirc > 1580 && xCirc < 1660 && yCirc > 167.5 & yCirc < 247.5) {
			circGoalColor = "#0f0";
				if (hasLetGo == true) {
		   circGoalAch = 1;
		} 
		   ...
		   circGoalAch = 0;
				...
				if (xTria > 1580 && xTria < 1660 && yTria > 937.5 & yTria < 1017.5) {
			triaGoalColor = "#0f0";
					if (hasLetGo == true) {
		   triaGoalAch = 1;
		}
		...
		if (rectGoalAch > 0 && circGoalAch > 0 && triaGoalAch > 0) {
