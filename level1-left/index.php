...
ctx.beginPath();
ctx.arc(300, 207.5, 125, 0, 2 * Math.PI);
ctx.lineWidth = 10;
...
ctx.beginPath();
ctx.moveTo(175, 997.5);
ctx.lineTo(425, 997.5);
ctx.lineTo(300, 997.5 - height2);
ctx.closePath();
ctx.lineWidth = 10;
ctx.globalAlpha = 1;
...
var countdown = setInterval(function() {
  ...
  if (seconds >= 600) {
	  ...
	  localStorage.setItem("shapesLeftScore", seconds);
	    exerStarted = 0;
...
drawTria(xTria, yTria);
		...
		if (xRect > 160 && xRect < 240 && yRect > 400 & yRect < 480) {
			rectGoalColor = "#0f0";
			if (hasLetGo == true) {
		   rectGoalAch = 1;
			}
		   ...
		   rectGoalAch = 0;
			...
			if (xCirc > 260 && xCirc < 340 && yCirc > 167.5 & yCirc < 247.5) {
			circGoalColor = "#0f0";
				if (hasLetGo == true) {
		   circGoalAch = 1;
		} 
		   ...
		   circGoalAch = 0;
				...
				if (xTria > 260 && xTria < 340 && yTria > 937.5 & yTria < 1017.5) {
			triaGoalColor = "#0f0";
					if (hasLetGo == true) {
		   triaGoalAch = 1;
		}
